...
int SDLInputContext::addGameController(int index)
{
	...
	auto controller = std::make_shared<SDLController>(SDL_JoystickNumAxes(joy), SDL_JoystickNumButtons(joy), id, device);
...
int SDLInputContext::removeGameController(int id)
{
	...
	{
		auto controller = std::static_pointer_cast<SDLController>(m_controllers[i]);
